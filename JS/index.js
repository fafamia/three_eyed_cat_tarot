...
const teacherList = document.querySelector(".teacher-menu");
//pc-teacher區域輪播 開始
function teacherChange() {
    ...
    //左鍵事件
    findId("team-left-arrow").onclick = function () {
        //確保curIndex在0-2的範圍內
        curIndex = (curIndex - 1 + 3) % 3;
        teacherList.style.left = - curIndex * 33.9 + '%';
    }

    //右鍵事件
    findId("team-right-arrow").onclick = function () {
...
//phone-teacher 輪播開始
function phoneTeacherChange() {
    let startX;

    teacherList.addEventListener('touchstart', function (event) {
    ...
    teacherList.addEventListener('touchmove', function (event) {
    ...
    // 處理滑動結束後的操作
    teacherList.addEventListener('touchend', function (event) {
        // 根據滑動距離和方向判斷要執行的操作
        if (Math.abs(deltaX) > 1) { // 設定一個閾值，例如20像素，確保水平滑動足夠長
            if (deltaX < 0 && curIndex < 4) {
                // 向右滑動
                curIndex++;
                teacherList.style.left = - curIndex * 75 + '%';
...
function productsChange() {
    ...
    //左鍵事件
    findId("products-l-btn").onclick = function () {
        // 確保 curIndex 在 0 到 5之間
        curIndex = (curIndex - 1 + 6) % 6;
        productsList.style.left = - curIndex * 33.9 + '%';
    }

    //右鍵事件
    findId("products-r-btn").onclick = function () {
        curIndex = (curIndex + 1) % 6;
        productsList.style.left = - curIndex * 33.9 + '%';
    }
}
//pc-hotproducts 輪播結束
//phone-hotproducts 輪播開始
function phoneProductsChange() {
    let startX;

    productsList.addEventListener('touchstart', function (event) {
        startX = event.touches[0].clientX;
    });
    ...
    productsList.addEventListener('touchmove', function (event) {
        deltaX = event.touches[0].clientX - startX;
        event.preventDefault(); // 阻止默認行為
    });
    ...
    // 處理滑動結束後的操作
    productsList.addEventListener('touchend', function (event) {
        // 根據滑動距離和方向判斷要執行的操作
        if (Math.abs(deltaX) > 1) { // 設定一個閾值，例如20像素，確保水平滑動足夠長
            if (deltaX < 0 && curIndex < 7) {
                // 向右滑動
                curIndex++;
                productsList.style.left = - curIndex * 75 + '%';
            } else if (deltaX > 0 && curIndex > 0) {
                // 向左滑動
                curIndex--;
                productsList.style.left = - curIndex * 75 + '%';
